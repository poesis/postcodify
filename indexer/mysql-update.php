...
echo '[Step 1/4] 도로 목록 및 행정구역 데이터를 읽어들이는 중 ... ' . "\n\n";
...
echo '[Step 2/4] 영문 번역 데이터를 읽어들이는 중 ... ' . "\n\n";
...
echo '[Step 3/4] 신규 도로명의 영문 명칭을 읽어들이는 중 ... ' . "\n\n";
...
$ps_synonym_select = $db->prepare('SELECT 1 FROM postcodify_keywords_synonyms WHERE original_crc32 = ?');
$ps_synonym_insert = $db->prepare('INSERT INTO postcodify_keywords_synonyms (original_crc32, canonical_crc32) VALUES (?, ?)');
...
foreach ($files as $filename)
{
    ...
    while ($line = trim(fgets($fp)))
    {
        ...
        if (!isset($english_synonyms[$road_name_ko]))
        {
            $english_synonyms[$road_name_ko] = true;
            $english_road_name = preg_replace('/[^a-z0-9]/', '', strtolower($road_name_en));
            $korean_road_name = get_canonical($road_name_ko);
            $ps_synonym_select->execute(array(crc32_x64($english_road_name)));
            if (!$ps_synonym_select->fetchColumn())
            {
                $ps_synonym_insert->execute(array(crc32_x64($english_road_name), crc32_x64(get_canonical($korean_road_name))));
            }
        }
    }
...
unset($ps_synonym_select);
unset($ps_synonym_insert);
echo "\n";
...
foreach ($files as $filename)
{
    ...
    while ($line = trim(fgets($fp)))
    {
        ...
        elseif ($change_reason == 31)
        {
            ...
            $english .= "\n" . ($is_basement ? 'Jiha ' : '') . $num_major . ($num_minor ? ('-' . $num_minor) : '') . ', ' . $english_cache[$road_name];
            $english .= "\n" . ($is_mountain ? 'San ' : '') . $jibeon_major . ($jibeon_minor ? ('-' . $jibeon_minor) : '') . ', ' . $english_cache[$dongri];
