...
버그 신고, 그 밖의 문의, 유료 기술지원 상담은 root@poesis.kr로 연락 주시기 바랍니다.
...
무료 예제 서버는 도메인당 하루 100회, 클라이언트 IP당 하루 100회 내외의 쿼리수 제한이 있으므로 대규모 웹사이트에 직접 적용하기는 곤란합니다.
...
서버 API를 구동하려면 PHP 5.3.7 이상, MySQL 5.1 이상 (또는 MariaDB), InnoDB, UTF-8 환경이 필요합니다.
...
1. 검색어 테이블이 큰 용량을 차지합니다. 지번 관련 검색어는 1600만 건, 도로명 관련 검색어는 1000만 건 내외를 추출합니다.
   주소를 저장하는 메인 테이블까지 합치면 5-6GB의 용량이 필요합니다.
