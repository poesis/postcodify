...
라이선스는 GPLv3을 따릅니다. 누구나 자유롭게 사용, 변형, 배포 또는 판매하실 수 있으나,
변형하여 배포 또는 판매하는 경우에는 반드시 상대방에게 소스코드를 공개하셔야 합니다.
이 경우 가능하면 GitHub을 통해 공개하셔서 더 많은 사람들이 개선된 프로그램을 사용할 수 있도록 해주시면 감사하겠습니다.

PHP 프로그램은 언어의 특성상 소스코드 그대로 배포 또는 판매하는 경우가 많습니다.
이 경우 소스코드를 제공했으므로 라이선스의 조건을 충족한 것으로 보겠습니다.
그러나 ionCube, Zend Guard 등으로 암호화 또는 난독화하여 판매하는 경우 라이선스에 저촉될 수 있습니다.

GPLv3 라이선스는 프로그램에만 적용되며, 프로그램을 사용하여 생성한 "결과물"에는 적용되지 않으므로
이 프로그램으로 생성한 우편번호 DB를 변형, 배포, 판매 또는 그 밖의 상업적 목적으로 사용하는 것은 자유입니다.
(애초에 정부에서 제공한 데이터이지, 제가 드린 것이 아니잖아요.)
...
아래의 설명대로 우편번호 DB를 자체 서버에 설치한 경우에는
반드시 `api` 설정을 변경하시기 바랍니다.

#### 서버측 검색 API ####

무료 예제 서버는 도메인당 하루 1000회, 클라이언트 IP당 하루 1000회 내외의 쿼리수 제한이 있으므로 대규모 웹사이트에 직접 적용하기는 곤란합니다.
이 경우 아래의 서버측 검색 API와 인덱서를 사용하여 직접 검색서버를 구축하셔야 합니다.
...
서버측 검색 API는 PHP/MySQL 스크립트로 제공됩니다.

서버 API를 구동하려면 PHP 5.2 이상, MySQL 5.1 이상 (또는 MariaDB), PDO, InnoDB, UTF-8 환경이 필요합니다.
그 이하의 버전에서도 실행이 가능할 수 있으나, 성능 및 호환성을 보장할 수 없습니다.
빠른 검색을 위해 InnoDB의 버퍼 크기를 최소 1GB, 가능하면 총 DB 크기의 절반 이상으로 설정하시기 바랍니다.
...
[우체국](http://www.epost.go.kr/search/zipcode/newAddressDown.jsp)에서 제공하는 사서함 정보를 분석하고
검색 키워드를 추출하여 MySQL DB에 입력하는 프로그램입니다.

포에시스의 우편번호 검색 프로그램은 아래와 같은 장점이 있습니다.

1. 곧 제공이 중단될 매칭테이블이 아닌 "유통자료개선" (도로명코드/주소/지번/부가정보) 형태의 자료를 사용합니다.
   또한 정부에서 제공하는 데이터 구조를 그대로 `LOAD DATA INFILE`하지 않고, 검색 결과 표시에 최적화된 형태로 가공하여 관련지번, 건물명 등을 한눈에 볼 수 있도록 합니다.
...
1. 검색어 테이블이 큰 용량을 차지합니다. 지번 관련 검색어는 1500만 건, 도로명 관련 검색어는 1000만 건 내외를 추출합니다.
   주소를 저장하는 메인 테이블과 인덱스 용량까지 합치면 5GB 내외의 공간을 차지합니다.
...
위와 같이 총 44개의 파일이 필요합니다. **우체국 매칭테이블은 사용하시면 안됩니다.**
...
실행되는 동안에는 아래와 같은 메시지가 출력됩니다.
    ...
    [Step 1/7] 테이블과 프로시저를 생성하는 중 ... 
    
    [Step 2/7] 도로 목록을 메모리에 읽어들이는 중 ... 
    
      -->  도로명코드_전체분.zip ...    348,947
    
    [Step 3/7] 쓰레드를 사용하여 "주소" 파일을 로딩하는 중 ... 
    ...
    [Step 4/7] 쓰레드를 사용하여 "지번" 파일을 로딩하는 중 ... 
    ...
    [Step 5/7] 쓰레드를 사용하여 "부가정보" 파일을 로딩하는 중 ... 
    ...
    [Step 6/7] 사서함 데이터를 로딩하는 중 ... 
    
      -->  newaddr_pobox_DB.zip ... 
    
      <--  경과 시간 : 1시간 40분 3초
    
    [Step 7/7] 인덱스를 생성하는 중. 아주 긴 시간이 걸릴 수 있습니다 ... 
