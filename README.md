...
서버 API를 구동하려면 PHP 5.2 이상, MySQL 5.1 또는 SQLite 3.6 이상, 그리고 mbstring 모듈이 필요합니다.
...
MySQL 덤프는 MariaDB 5.x 버전에서 생성하며, 실제 DB 서버에 복구하는 과정을 거쳐서 사용하셔야 합니다.
SQLite DB는 SQLite 3.x 버전에서 생성하며, 압축만 풀어서 그대로 사용 가능합니다.
...
1. 검색어 테이블이 큰 용량을 차지합니다. 지번 관련 검색어는 1500만 건, 도로명 관련 검색어는 1000만 건 내외를 추출합니다.
   주소를 저장하는 메인 테이블과 인덱스 용량까지 합치면 최소 5GB에서 최대 8GB의 공간을 차지합니다. (테이블 단편화 정도에 따라 다름)
