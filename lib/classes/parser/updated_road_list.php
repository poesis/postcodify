...
class Postcodify_Parser_Updated_Road_List extends Postcodify_TextFileReader
{
    // 생성자에서 문자셋을 지정한다.
    ...
    public function read_line($delimiter = '|')
    {
        ...
        $road_name_english = trim($line[3]);
        $sido = trim($line[5]);
        $sido_english = str_replace('-si', '', trim($line[15]));
        $sigungu = trim($line[6]);
        $sigungu_english = trim($line[16]);
        $eupmyeon = trim($line[9]);
        $eupmyeon_english = trim($line[17]);
